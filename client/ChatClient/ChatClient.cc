            ...
            if (shortchat) {
                ...
                if (chat_menu_index != -1){
                    ...
                    if (index == 4) {
                        tcsetattr(STDIN_FILENO, TCSANOW, &oldt);
                        if (inchat) {
                            handle_rechatsendfile(
                                current_chat);
                        } else {
                            handle_regroupchatsendfile(
                                groupname);
                        }
                        std::string s1 = GREEN;
                        s1 += "[" + name + "]:" + RESET;
                    ...
                    if (index == 5) {
                        tcsetattr(STDIN_FILENO, TCSANOW, &oldt);
                        if (inchat) {
                            handle_rechatloadfile(
                                current_chat);
                        } else {
                            handle_regroupchatloadfile(
                                groupname);
                        }
                        std::string s1 = GREEN;
                        s1 += "[" + name + "]:" + RESET;
                    ...
                    if (index == 6) {
                        ...
                        std::vector<std::string> chat;
                        if (inchat) {
                            chat = SQ.getfriendchat(
                                name, chatfriendname);
                            if (chat.empty()) {
                                handle_getfriendchat(
                                    current_chat);
                                chat = SQ.getfriendchat(
                                    name, chatfriendname);
                            }
