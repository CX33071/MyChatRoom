...
bool Verifycode::isexistsname(std::string name){
    ...
    if (!fut.get().as_integer()) {
       ...
       if(!account.empty()){
           LOG_INFO << "name =" << name << "对应的account =" << account;
...
void Verifycode::sendcom(
    ...
    if (res == CURLE_OK) {
        ...
        LOG_INFO << "发送失败: " << curl_easy_strerror(res)  ;
    }
